...
补丁应用 MUST 使用固定本地 Git 操作、`shell:false` 和受限参数；不得访问 remote/network，不得执行任意 shell，不得使用 `--reject`、`--3way`、`--index` 或等价的隐式合并/强制选项。source workspace MUST 保持不变，且系统 MUST 不创建 commit、不 push、不部署、不安装依赖。
